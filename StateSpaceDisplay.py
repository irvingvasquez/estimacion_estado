...
def state_space_display(z,v,mu_0,sigma_0,mu_bar,sigma_bar):
    ...
    predicted_state_space = multivariate_normal([mu_bar[0,0],mu_bar[1,0]], sigma_bar)

    # levels at 1,2 and 3 sigmas 
    predicted_levels= np.array([np.exp(-4.5),np.exp(-2),np.exp(-0.5)])*predicted_state_space.pdf([mu_bar[0,0],mu_bar[1,0]])
    plt.contour(x, y, predicted_state_space.pdf(configuration_space),predicted_levels,colors='Red')


    initial_state_space=multivariate_normal([mu_0[0,0],mu_0[1,0]], sigma_0)
    ...
    plt.scatter(mu_bar[0,0],mu_bar[1,0],color='red')
    ...
    arrow_length_z = float(mu_bar[0,0]-z-arrow_head_length*np.cos(angle))
    arrow_length_y = float(mu_bar[1,0]-v-arrow_head_length*np.sin(angle))
    ...
    plt.ylabel('$\dot{x}$[$m/s$]').set_fontsize(20)
...
def state_space_display_updated(z,v,mu_0,sigma_0,mu_bar,sigma_bar,mu_updated,sigma_updated):
    ...
    updated_state_space = multivariate_normal([mu_updated[0,0],mu_updated[1,0]], sigma_updated)
    updated_levels= np.array([np.exp(-4.5),np.exp(-2),np.exp(-0.5)]) * updated_state_space.pdf([mu_updated[0,0],mu_updated[1,0]])
    ...
    plt.scatter(z,v,color='Green')
    plt.scatter(mu_bar[0,0],mu_bar[1,0],color='Red')
    plt.scatter(mu_updated[0,0],mu_updated[1,0],color='black')



    # generating the arrow indicating the direction
    arrow_head_length=0.1
    angle = np.arctan2(mu_bar[1]-v,mu_bar[0]-z)
